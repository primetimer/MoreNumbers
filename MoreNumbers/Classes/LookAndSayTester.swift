...
//		if self.name == "H" { return ans }
	...
	static func LookPrevious(say: String) -> (prev: String, speakable : Bool) {
				let c = Array(say)
		let count = c.count
		if count == 0 { return ("unspeakable",false) }
		var speakable = true

		var ans = ""
		var prev = String("0")
		for i in stride(from: 0, to: count, by: 2) {
			guard let ccount = Int(String(c[i])) else { break }
			if i+1 >= count || ccount == 0  {
				ans = ans + "?"
				speakable = false
				break
			}
		...
		let check = ConwayElem.LookandSay(look: ans)
		if check != say {
			ans = ans + "?"
			speakable = false
		}
		return (ans,speakable)
	}
}
class ConwayActive {
...
public class LookAndSayTester : NumTester {
    ...
    func Say(n: BigUInt) -> SayAndAnswer? {
		...
		let a = Array(s)
		for i in 0..<a.count {
			formatted = formatted + String(a[i])
			let nextc = i+1>=a.count ? Character("0") : a[i+1]
			if a[i] == nextc {
				count = count + 1
				continue
			}
